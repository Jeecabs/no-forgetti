...
The background reviewer stages at most one create/patch/archive proposal per review. New creates auto-approve; inspect patch/archive proposals with `/project-skills pending`, then approve or reject explicitly. `/project-skills view <name>` reads a skill and `/project-skills edit <name>` opens the built-in multiline editor and saves a revision. Relevant skills are retrieved automatically from the current prompt; use the `project_skill` tool for explicit list/read access. Explicit reads return the full stored playbook. They remain external to the repository.
