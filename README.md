...
- **Maintenance** happens inside that same atomic mutation. Background reviews can grow memory to the 4,500-character working target. At or above it, reviews must consolidate, remove stale or low-value facts, leave usage unchanged, or safely no-op. The 6,000-character hard limit reserves 1,500 characters for foreground writes.
...
`oldText` is a unique substring used by the foreground model tool. Background reviews target existing entries by stable ID. They can add, replace, remove, merge, or assess entries. Memory has a 4,500-character review target, a 6,000-character hard limit, and an 800-character per-entry limit. Exact duplicates are ignored. Reviews below the target cannot cross it. Reviews at or above the target cannot grow memory. Capacity-violating proposals safely no-op instead of entering a retrying failure state. Accepted changes form one atomic batch against final hard capacity and an exact base-branch digest.
