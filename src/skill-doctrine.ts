...
Bold terms are defined in the GLOSSARY section below; look them up there for the full meaning.
...
In-skill reference — a definition, rule, or fact in SKILL.md, consulted on demand. Often a legitimately flat peer-set (every rule of a review on one rung) — a fine arrangement, not a smell. This skill is all reference.
External reference — reference pushed out of SKILL.md into a separate file, reached by a context pointer, loaded only when the pointer fires. (Spans disclosed reference — a sibling file like GLOSSARY.md, still part of the skill — through fully external reference that lives outside the skill system and any skill can point at.)
...
const GLOSSARY = `The domain model for what makes a skill great. A skill exists to wrangle determinism out of a stochastic system; the root virtue is Predictability, and every term below is a lever on it. This is the disclosed reference for writing-great-skills.
